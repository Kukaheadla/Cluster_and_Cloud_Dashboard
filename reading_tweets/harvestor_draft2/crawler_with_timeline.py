...
def main_search(tweet_lst, id_lst, search_no, client):
...
def main_stream(streaming_no, bearer_token):
    ...
    return [client.result, client.tweet_id_lst, client.count, client.total_tweets_read, client.end_early, client.tweet_user_id_lst]

def timeline_search(timeline, user_id_lst, user_timeline, client, id_lst, streaming_no):
    counter = 0
    total_tweets_read = 0
    for val in user_id_lst:
        ...
        tmp_timeline = client.get_users_tweets(val, max_results=user_timeline, tweet_fields = tweet_fields, user_fields = user_fields)

        for item in tmp_timeline.data:
            tmp = dict(item)
            #Check to see if the tweet has been posted before:
            if str(tmp["id"]) not in id_lst:
                #print(tweet.__repr__())
...
if __name__ == "__main__":
    ...
    for value in ids: 
        ...
        ##Now can use the user_ids to search up timelines:
        #Limit of 3200
        #Limit to 100 per user.
        ...
        if total_tweets_obtained < streaming_no + search_no:
            ...
            if total_tweets_obtained < streaming_no:
                search_no += abs(streaming_no - total_tweets_obtained)

            elif total_tweets_obtained > streaming_no:
                search_no -= abs(total_tweets_obtained - streaming_no)

            print("Now run the search API")
            search_result = main_search(tmp_search, id_lst, search_no, client)
